...
if (flags.has('--preview') || args.some(a => a.startsWith('--at='))) {
  ...
  const shots = explicit
    ? explicit.split(',').filter(Boolean).map((s, i) => ({ label: `at-${s}s`, t: Math.round(Number(s) * 1000) }))
    // 0.82 through the shot: late enough that the beats it triggers have
    // settled, early enough to be inside the shot that scheduled them.
    : SHOTS.map(s => ({ label: s.id, t: Math.round(s.start + s.dur * 0.82) }));
