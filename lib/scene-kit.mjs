...
// ---------------------------------------------------------------- morphing
/**
 * Build a smooth path through anchor points, deriving the bezier controls
 * (Catmull-Rom).
 *
 * This is the thing that makes the set's core mechanic tractable. Hand-placing
 * control points for two shapes that have to lerp control-for-control is
 * miserable and error-prone; define each mark as *anchors only* and two marks
 * with the same anchor count lerp anchor-for-anchor, with every in-between
 * guaranteed to be one connected stroke.
 *
 * `tension` scales the derived controls. 1 is a relaxed Catmull-Rom, which
 * balloons at a corner; below about 0.3 the corners come to a point. Anything
 * carved, ruled or printed needs a low tension somewhere — episode 4 needed it
 * for a sharp's crossbars, episode 5 for the crenellations of a rook — so it is
 * a parameter rather than a constant.
 */
export function smooth(anchors, { closed = false, tension = 1 } = {}) {
  const a = anchors;
  if (a.length < 2) throw new Error('smooth() needs at least two anchors');
  const at = i => a[closed ? (i + a.length) % a.length : Math.max(0, Math.min(a.length - 1, i))];
  const path = new P().M(a[0][0], a[0][1]);
  const last = closed ? a.length : a.length - 1;
  const k = tension / 6;
...
/**
 * Interpolate between two anchor sets and smooth the result.
 *
 * Equal anchor counts are enforced rather than assumed: unequal counts are the
 * topology failure the gate is meant to catch, and it is far cheaper to hear
 * about it here than to watch a shape tear in a rendered frame.
 *
 * What this cannot check for you: **each corresponding piece must keep its
 * direction of travel.** Two shapes can have identical anchor counts and still
 * fold through themselves mid-morph if a run is indexed in opposite directions
 * — a box whose base goes right-to-left against a crossbar that goes
 * left-to-right. If an in-between looks knotted, reverse one run.
 */
export function morph(from, to, p, opts = {}) {
  return smooth(blend(from, to, p), opts);
}

/** Lerp two equal-length anchor sets without smoothing them yet. */
export function blend(from, to, p) {
  if (from.length !== to.length) {
    throw new Error(`morph() needs equal anchor counts, got ${from.length} and ${to.length}`);
  }
  const e = clamp01(p);
  return from.map((pt, i) => [lerp(pt[0], to[i][0], e), lerp(pt[1], to[i][1], e)]);
}

/**
 * Walk one shape along a list of states. `x` runs 0 .. states.length - 1.
 *
 * A derivation with a documented intermediate — an elephant, then the abstract
 * block a carver left of it, then the bishop Europe read out of the block — is
 * one mark in three states, not two morphs bolted together. Passing the whole
 * chain to one function is what keeps it a single continuous stroke, and it is
 * what lets a single parameter drive the whole history.
 */
export function along(states, x, opts = {}) {
  const n = states.length - 1;
  const u = clamp01(x / n) * n;
  const i = Math.min(Math.floor(u), n - 1);
  return smooth(blend(states[i], states[i + 1], u - i), opts);
}

/**
 * A mirror-symmetric closed ring, from one apex, one half, and the other apex.
 *
 * Nearly everything this set draws is an object made on a lathe or a page — a
 * chess piece, a clef, a pip — so half of every anchor list was being typed
 * twice and drifting by a pixel or two when it was edited. Writing the right
 * half once also guarantees the mirrored indices line up, which is half of the
 * topology rule `morph()` cannot check.
 */
export function mirrorRing(top, right, bottom) {
  return [top, ...right, bottom, ...right.slice().reverse().map(([x, y]) => [-x, y])];
}

/** Anchors of a circle, for morphing against any other same-count mark. */
export function ring(cx, cy, r, count = 8, phase = -Math.PI / 2) {
...
/**
 * The standard reveal: an object is drawn, a copy of it detaches and travels
 * across the page while it morphs, and the object itself stays exactly where it
 * was at a low opacity with a dashed tie to what it became.
 *
 * This is two rules the films kept relearning, combined into one call.
 *
 * The source never leaves (craft §7). Fading it out is the single commonest
 * composition failure here: the mark lands early, the narrator keeps talking,
 * and the page sits two-thirds empty for six seconds. Thinning it in place
 * fixes that *and* keeps the argument visible, since the claim is "this is
 * that" and it is much weaker with half of it gone.
 *
 * And the morph travels. Drawing the source at one place, the mark at another,
 * and morphing in place at the second is a cut with extra steps — the object
 * teleports, and the one thing this medium can do is thrown away.
 *
 * While it is alone on the page the object holds the centre (craft §9) and
 * slides out to `from` only as its copy leaves — it makes room rather than
 * having started in a corner waiting. Call this for the whole stage, including
 * before the morph begins, and that is handled.
 *
 * `draw(t, cx, opacity)` renders the mark at morph position t, centred on cx.
 */
export function travel(draw, p, { from = 660, to = 1320, y = 560, alone = W / 2, ghost = 0.34, inset = 250, shrink = 1 } = {}) {
  const e = easeInOut(clamp01(p));
  const heldX = lerp(alone, from, e);
  const scale = lerp(1, shrink, e);
  const at = (inner, cx) => (scale === 1 ? inner
    : `<g transform="translate(${cx.toFixed(1)} ${y}) scale(${scale.toFixed(3)}) translate(${(-cx).toFixed(1)} ${-y})">${inner}</g>`);
  const held = at(draw(0, heldX, lerp(1, ghost, e)), heldX);
  if (e <= 0.01) return held;
  // The copy leaves on easeOut rather than the shared easeInOut, so it clears
  // the source early instead of loitering beside it. A wide mark — episode 7's
  // C I Ɔ is 540px across — otherwise overlaps its own ghost around the middle
  // of the move and the page reads as a pile-up.
  const copyX = lerp(heldX, to, easeOut(e));
  const tie = ink(new P().M(from + inset, y).L(to - inset, y), Math.min(1, e * 2),
    { w: 3, color: INK_SOFT, dash: '12 10', opacity: 0.55 });
  return held + tie + at(draw(e, copyX, 1), copyX);
}

/**
 * A detail lifted out of a larger drawing.
 *
 * `travel()` is the figure for a free-standing object: it owns the page centre,
 * makes room, and slides sideways along one line. A detail *inside* a bigger
 * drawing cannot do any of that. A notch belongs to its stick, a pip to its
 * card, a capital to its column — sliding the parent aside to make room would
 * undo the very thing the shot is claiming, and the detail starts at the
 * parent's scale rather than the page's.
 *
 * So this is the same argument with the parent held still. A copy grows out of
 * the detail at its own scale, travels to wherever on the page it belongs —
 * diagonally, which `travel()` cannot do — and the whole parent thins where it
 * stands, tied to what left it. Craft §7 either way: the source never leaves.
 *
 * `draw(t, cx, cy, opacity)` renders the detail, t running 0 (in place, source
 * scale) to 1 (arrived, page scale). `parent(opacity)` renders what it came out
 * of, and thins with it.
 */
export function detach(draw, p, {
  from, to, fromY, toY, ghost = 0.3, parent = null,
} = {}) {
  const e = easeInOut(clamp01(p));
  const dim = lerp(1, ghost, e);
  const back = (parent ? parent(dim) : '') + draw(0, from, fromY, dim);
  if (e <= 0.01) return back;
  const cx = lerp(from, to, e);
  const cy = lerp(fromY, toY, e);
  const tie = ink(new P().M(from, fromY).L(cx, cy), 1,
    { w: 3, color: INK_SOFT, dash: '12 10', opacity: 0.5 * Math.min(1, e * 3) });
  return back + tie + draw(e, cx, cy, 1);
}
